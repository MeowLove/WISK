# WISK 3.0.0 Release Testing Guide

## English (authoritative)

WISK 3.0.0 is the first public release of the new WISK implementation. It is a
regular GitHub Release, not marked as a pre-release, and is intended for rapid
feedback-driven iteration. This unsigned release has not completed isolated
system-change acceptance. Get the packages from the official release page:
<https://github.com/MeowLove/WISK/releases/tag/v3.0.0>.
...
export wording. We plan to iterate rapidly on reproducible community feedback.
Do not approve elevation prompts during ordinary testing. Do not click Apply
on a daily-use, work, or production computer.

Testing real system changes requires a disposable, isolated Windows 11 test
environment with a recoverable snapshot. This release has not completed full
...
When reporting a problem, use the [WISK 3.0 issue template](https://github.com/MeowLove/WISK/issues/new?template=bug_report.md)
and include WISK version, package type, Windows edition and build, reproduction
steps, expected result, actual result, and the relevant task ID if visible.
Diagnostic exports attempt to redact known account,
...
WISK 3.0.0 是全新 WISK 实现的首个公开 Release，不标记为 Pre-release，
并计划根据社区反馈快速迭代。此版本尚未签名，也尚未完成隔离环境中的系统修改验收。
请从官方 Release 页面下载：
<https://github.com/MeowLove/WISK/releases/tag/v3.0.0>。
...
设置配置与取消、计划预览、历史状态文字和诊断导出提示。普通测试过程中不要
...
我们计划根据可复现的社区反馈快速迭代 3.0。请使用 [WISK 3.0 问题反馈模板](https://github.com/MeowLove/WISK/issues/new?template=bug_report.md)
提交问题，并包含 WISK 版本、包类型、Windows 版本和内部版本号、复现步骤、预期结果、实际结果，以及界面上可见的任务 ID。

诊断导出会尝试隐藏已知账户、计算机、配置档、
