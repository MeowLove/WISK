...
public sealed class ControlledExtensionInstallerTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "WISK wisk-install-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "WISK wisk-install-invalid-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
