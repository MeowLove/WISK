...
public sealed class DiagnosticExporterTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "windows-initializer-diagnostics-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "windows-initializer-diagnostics-path-" + Guid.NewGuid().ToString("N"));
        var path = Path.Combine(root, "run.json");
