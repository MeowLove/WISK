...
public sealed class ExecutionEngineTests
{
    ...
    {
        var store = new AtomicJsonStateStore(Path.Combine(Path.GetTempPath(), "windows-initializer-tests"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "windows-initializer-corrupt-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "windows-initializer-corrupt-history-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        await File.WriteAllTextAsync(Path.Combine(root, "broken.json"), "{ invalid");
        try
        {
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "windows-initializer-incomplete-history-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "windows-initializer-lock-" + Guid.NewGuid().ToString("N"));
